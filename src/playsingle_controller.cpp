...
/*
 * TODO:
 * The code to compute the resurn value of play_scenario was chopped a little
 * 
 * Changes:
 * 
 */
...
{
	assert(gamestate().in_phase(game_data::TURN_STARTING_WAITING, game_data::TURN_PLAYING, game_data::TURN_ENDED, game_data::GAME_ENDED));
...
level_result::type playsingle_controller::play_scenario(const config& level)
{
	...
	try {
		...
		// I don't understand this test_result. In particular: what is the difference between "test_invalid", "result_not_set" and an empty string.
		return level_result::get_enum(get_end_level_data().test_result).value_or(level_result::type::victory);
...
bool playsingle_controller::should_return_to_play_side() const
{
	if(player_type_changed_ || is_regular_game_end()) {
		return true;
	} else if(gamestate().in_phase(game_data::TURN_PLAYING) || replay_controller_.get() != 0 || current_team().is_network()) {
		return false;
	} else {
		return true;
	}
